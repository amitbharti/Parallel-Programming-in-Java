...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
            final Student[] studentArray) {
        Double averageSum = Stream.of(studentArray).parallel().filter(Student::checkIsCurrent).
                map(Student::getAge).reduce(0D,(o1, o2) -> o1+o2);
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        Optional<Map.Entry<String, Long>> entry = Stream.of(studentArray).parallel().filter(s -> !s.checkIsCurrent())
                .map(Student::getFirstName).collect(Collectors.groupingBy(name ->name,Collectors.counting())).entrySet().stream().max(Comparator.comparing(Map.Entry::getValue));
        if (entry.isPresent()) {
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
       int failedCount=(int) Stream.of(studentArray).parallel().
               filter(s -> s.getAge()>20).filter(s -> s.getGrade()<65 && !s.checkIsCurrent()).count();
        return failedCount;
    }
}
